...
import com.example.nback.engine.decodeOutcomes
...
data class HistoryRecord(
    ...
    @ColumnInfo(defaultValue = "''") val numberOutcomes: String = "",
) {
    fun outcomes() = if (rulesVersion < 3) emptyMap() else activeTypes(modeMask).associateWith { decodeOutcomes(encodedOutcomes(it)) }
    private fun encodedOutcomes(type: StimulusType) = when (type) {
        StimulusType.POSITION -> positionOutcomes; StimulusType.COLOUR -> colourOutcomes; StimulusType.NUMBER -> numberOutcomes
    }
...
private const val INVALID_V2_ROW = "typeof(id) != 'text' OR trim(id) = '' OR " +
    "typeof(completedAt) != 'integer' OR completedAt < -62135596800000 OR completedAt > 253402300799999 OR " +
    "typeof(level) != 'integer' OR level NOT BETWEEN 1 AND 3 OR " +
...
private const val INVALID_V3_ROW = "typeof(id) != 'text' OR trim(id) = '' OR " +
    "typeof(completedAt) != 'integer' OR completedAt < -62135596800000 OR completedAt > 253402300799999 OR " +
    "typeof(level) != 'integer' OR level NOT BETWEEN 1 AND 3 OR " +
    "typeof(rulesVersion) != 'integer' OR rulesVersion NOT IN (1,2,3) OR " +
    "typeof(modeMask) != 'integer' OR modeMask NOT BETWEEN 1 AND 7 OR (rulesVersion = 1 AND modeMask != 1) OR " +
    "typeof(hits) != 'integer' OR hits NOT BETWEEN 0 AND 6 OR " +
    "typeof(misses) != 'integer' OR misses NOT BETWEEN 0 AND 6 OR " +
    "typeof(falseAlarms) != 'integer' OR falseAlarms NOT BETWEEN 0 AND 14 OR " +
...
class RoomHistoryStore(private val context: Context, private val file: File) : HistoryStore {
    ...
    companion object {
        ...
        private val outcomeColumns = listOf("positionOutcomes", "colourOutcomes", "numberOutcomes")
        val MIGRATION_2_3 = object : Migration(2, 3) {
            override fun migrate(connection: SQLiteConnection) {
                connection.prepare("ALTER TABLE sessions ADD COLUMN intervalSeconds INTEGER NOT NULL DEFAULT 3").use { it.step() }
                outcomeColumns.forEach { name ->
                    connection.prepare("ALTER TABLE sessions ADD COLUMN $name TEXT NOT NULL DEFAULT ''").use { it.step() }
                }
            }
        }
