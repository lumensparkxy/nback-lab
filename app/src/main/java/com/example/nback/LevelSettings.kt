...
data class LoadedLevel(val level: Int = 2, val reset: Boolean = false)
interface LevelSettings {
    suspend fun load(): LoadedLevel
    suspend fun save(level: Int)
}
...
class StoredLevelSettings(
    ...
    override suspend fun load(): LoadedLevel {
        ...
        return LoadedLevel(if (!invalid && value is Int) value else 2, consumeCorruption() || invalid)
    }

    override suspend fun save(level: Int) {
        require(level in 1..3)
        store.edit { it[intPreferencesKey("selected_n")] = level }
    }
...
enum class SettingsNotice { RESET, LOAD_FAILED, SAVE_FAILED }
